...
import StoreSettings from '../../features/admin/StoreSettings';
import PaymentsTable from '../../features/admin/PaymentsTable'; // استيراد المكون الجديد
import { LayoutDashboard, Package, List, ShoppingBag, Users, Store, DollarSign, Settings, CreditCard } from 'lucide-react';
...
const AdminDashboardPage = () => {
  ...
  const tabs = [
    ...
    // ربط صفحة المدفوعات هنا بشكل صحيح
    { id: 'payments', label: 'المدفوعات', icon: CreditCard, component: PaymentsTable },
    ...
    { id: 'store_settings', label: 'إعدادات المتجر', icon: Settings, component: StoreSettings },
  ];
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-dark-bg flex flex-col md:flex-row animate-fade-in">
      <aside className="w-full md:w-64 bg-white dark:bg-dark-card shadow-md z-10 flex-shrink-0 transition-colors duration-300">
        <div className="p-6 border-b border-gray-100 dark:border-gray-700">
          <h2 className="text-xl font-bold text-primary-600 dark:text-primary-400">{t('nav.dashboard')}</h2>
        </div>
        <nav className="mt-6 px-4 space-y-2">
          {tabs.map((tab) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 font-medium ${activeTab === tab.id ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-400 shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-200'}`}
            >
      ...
      <main className="flex-1 p-4 sm:p-8 overflow-y-auto">
        <div className="max-w-7xl mx-auto">
          <header className="mb-8 flex justify-between items-center">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{tabs.find(t => t.id === activeTab)?.label}</h1>
          </header>
