...
const CartPage = () => {
  ...
  if (cartItems.length === 0) {
    return (
      <div className="container mx-auto px-4 py-20 flex flex-col items-center justify-center text-center bg-white dark:bg-dark-bg rounded-2xl">
        <motion.div
          initial={{ scale: 0.8, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ duration: 0.5 }}
          className="w-24 h-24 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mb-6 shadow-xl"
        >
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 xl:gap-12">
          ...
          <AnimatePresence>
            {cartItems.map((item) => (
              <motion.div
                ...
                layout
              >
                {/* استدعاء مكون CartItem - سيتم تحديثه لاحقاً */}
                <CartItem item={item} />
              </motion.div>
            ))}
          </AnimatePresence>
        ...
        <div className="lg:col-span-1">
          {/* استدعاء مكون OrderSummary - سيتم تحديثه لاحقاً */}
          <OrderSummary />
        </div>
      </div>
    </div>
